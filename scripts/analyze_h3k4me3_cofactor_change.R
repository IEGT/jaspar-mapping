...
usage <- function(status = 0L) {
    ...
    writeLines(c(
        ...
        "       --thresholds FILE.tsv --window NAME --output-prefix PATH [options]",
        "",
        ...
        "",
        "Options:",
        "  --signal FILE             Windowed H3K4me3/input Parquet",
        ...
        "  --window NAME             Predeclared signal window to analyze",
...
while (index <= length(arguments)) {
    ...
    if (option == "--series") {
        values$series <- c(values$series, arguments[[index]])
    } else {
...
required <- c(
    "signal", "tp73_evidence", "cofactor_maxima", "thresholds", "window",
    "output_prefix"
)
...
for (name in c("signal", "tp73_evidence", "cofactor_maxima", "thresholds")) {
...
signal <- duckdb_fread(paste0(
    ...
    "input_positive_bp FROM read_parquet(", sql_string(values$signal), ") ",
...
if (nrow(signal) == 0L || length(setdiff(required_signal, names(signal))) > 0L) {
    stop("selected H3K4me3 signal window is empty or incomplete", call. = FALSE)
}
available_series <- sort(unique(signal$series_id))
analysis_series <- if (length(values$series) > 0L) values$series else available_series
...
maxima <- duckdb_fread(paste0(
    ...
    "FROM read_parquet(", sql_string(values$cofactor_maxima), ") WHERE motif_id IN (",
...
if (maxima_provenance[, any(source_score_floor > min(negative_references))]) {
    stop(
        "cofactor scan floor is above a requested negative reference; ",
        "the negative class cannot be identified without bias",
        call. = FALSE
    )
}
...
for (motif_index in seq_len(nrow(thresholds))) {
    ...
    for (negative_reference in negative_references) {
        ...
        joined[, cofactor_negative := is.na(context_score) |
                   context_score < negative_reference]
        ...
        for (isoform in c("TA", "DN")) {
            ...
            for (series_id in analysis_series) {
                ...
                current <- joined[series_id == current_series]
                ...
                class_supported <- !overlap_invalid && eligible_count > 0L &&
                ...
                comparison_status <- if (overlap_invalid) {
                ...
                comparison_note <- if (overlap_invalid) {
                ...
                if (overlap_invalid) {
                    ...
                    total <- clustered_lm(
                        model_data,
                        as.formula(paste0(
                            "outcome ~ splines::ns(anchor_score, df = ",
                            values$spline_df, ") + cofactor_present"
                        )),
                ...
                if (overlap_invalid) {
                    ...
                    occurrence_summary <- model_data[, .N,
...
series_summary <- intensity[, .(
    ...
    motif_id, factor_name, positive_threshold, negative_reference_threshold, isoform
)]
...
run_config <- data.table(
    ...
    all_zero_anchor_policy = "retained",
