...
usage() {
    cat <<'EOF'
Usage: run_h3k4me3_genome_signal_finalize.sh --run-root DIR --source DIR
       --duckdb FILE [options]

Validate every nuclear-chromosome H3K4me3 package and atomically publish the
partitioned signal, GFP-referenced change table, summary, and DuckDB catalog.

Options:
  --run-root DIR       Prepared genome-wide H3K4me3 run
...
scratch="$scratch_base/h3-genome-finalize-${SLURM_JOB_ID:-manual}-$$"
mkdir -p "$scratch"

exec python3 "$source/scripts/manage_h3k4me3_genome_signal.py" finalize \
