...
for partition in \
    "motif_id=MA0001.1/score_mode=log_odds/pseudocount=1/chrom=1/strand=plus" \
    "motif_id=MA0001.1/score_mode=log_odds/pseudocount=1/chrom=1/strand=minus" \
    "motif_id=MA0001.1/score_mode=log_odds/pseudocount=1/chrom=2/strand=plus" \
    "motif_id=MA0001.1/score_mode=log_odds/pseudocount=1/chrom=2/strand=minus" \
    "motif_id=MA0001.1/score_mode=log2_relative_risk/pseudocount=1/chrom=1/strand=plus" \
    "motif_id=MA0002.1/score_mode=log_odds/pseudocount=1/chrom=1/strand=plus"
do
    mkdir -p "$dense_root/$partition"
done
...
(
    ...
    duckdb :memory: -bail -c "
        COPY (
            SELECT * FROM (
                VALUES
                    ('MA0001.1'::VARCHAR, 'TEST'::VARCHAR, 4::INTEGER,
                     2026::INTEGER, 'fixture'::VARCHAR),
                    ('MA0002.1'::VARCHAR, 'OTHER'::VARCHAR, 2::INTEGER,
                     2026::INTEGER, 'fixture'::VARCHAR)
        ...
        COPY (SELECT 100::BIGINT AS block_start,
          ...
          TO 'tables/jaspar2026/motif_score_dense/motif_id=MA0001.1/score_mode=log_odds/pseudocount=1/chrom=1/strand=plus/part-000000.parquet'
          (FORMAT PARQUET, COMPRESSION ZSTD);
        COPY (SELECT 100::BIGINT AS block_start,
                     [0.5::FLOAT, 4.0::FLOAT, -2.0::FLOAT, NULL::FLOAT] AS scores)
          TO 'tables/jaspar2026/motif_score_dense/motif_id=MA0001.1/score_mode=log_odds/pseudocount=1/chrom=1/strand=minus/part-000000.parquet'
          (FORMAT PARQUET, COMPRESSION ZSTD);
        COPY (SELECT 200::BIGINT AS block_start,
                     [3.5::FLOAT, -3.0::FLOAT] AS scores)
          TO 'tables/jaspar2026/motif_score_dense/motif_id=MA0001.1/score_mode=log_odds/pseudocount=1/chrom=2/strand=plus/part-000000.parquet'
          (FORMAT PARQUET, COMPRESSION ZSTD);
        COPY (SELECT 200::BIGINT AS block_start,
                     [5.0::FLOAT, 0.0::FLOAT] AS scores)
          TO 'tables/jaspar2026/motif_score_dense/motif_id=MA0001.1/score_mode=log_odds/pseudocount=1/chrom=2/strand=minus/part-000000.parquet'
          (FORMAT PARQUET, COMPRESSION ZSTD);
        COPY (SELECT 100::BIGINT AS block_start, [9.0::FLOAT] AS scores)
          TO 'tables/jaspar2026/motif_score_dense/motif_id=MA0001.1/score_mode=log2_relative_risk/pseudocount=1/chrom=1/strand=plus/part-000000.parquet'
          (FORMAT PARQUET, COMPRESSION ZSTD);
        COPY (SELECT 100::BIGINT AS block_start, [8.0::FLOAT] AS scores)
          TO 'tables/jaspar2026/motif_score_dense/motif_id=MA0002.1/score_mode=log_odds/pseudocount=1/chrom=1/strand=plus/part-000000.parquet'
...
grep -F $'MA0001.1\tTEST\tlog_odds\t1.0\t2\t-' <<<"$configs" >/dev/null || {
...
assert_equal "provenance columns or score formatting are wrong" \
    $'Chromosome\tFrom\tTo\tName\tScore\tStrand\tMotifID\tScoreMode\tPseudocount\tCoordinateMode\n2\t200\t204\tTEST\t3.500\t+\tMA0001.1\tlog_odds\t1.0\tbed\n2\t200\t204\tTEST\t5.000\t-\tMA0001.1\tlog_odds\t1.0\tbed' \
    "$provenance_export"
