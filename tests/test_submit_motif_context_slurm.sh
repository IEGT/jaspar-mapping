...
printf 'synthetic threshold registry\n' > "$temporary/operating.parquet"
...
rendered=$("$repository_root/scripts/submit_motif_context_slurm.sh" \
    ...
    --operating-threshold-parquet "$temporary/operating.parquet" \
    --operating-threshold-set-id synthetic_operating_v1 \
...
grep -Fq -- '--job-name=tp73_context_v10' <<< "$rendered"
...
grep -Eq $'\t10\t[0-9]+\t[0-9a-f]{64}\t[1-9][0-9]*\t[0-9a-f]{64}\tsynthetic_operating_v1\tensembl_113\ttss_upstream_2000_downstream_500_v1\t2000\t500\ttes_upstream_500_downstream_2000_v1\t500\t2000\tcofactor_context$' \
    "$temporary/run/plan/context_tasks.tsv"
cmp -s "$temporary/operating.parquet" \
    "$temporary/run/inputs/operating_thresholds.parquet"
...
grep -Fq $'5\tX\tMA0005.1\tband' \
    "$temporary/batched/plan/context_tasks.tsv"
awk -F '\t' 'NR > 1 && ($6 != 10 || $7 != 0 || $8 != "none" \
    || $9 != 0 || $10 != "none" || $11 != "none" \
    || $12 != "ensembl_113" \
    || $13 != "tss_upstream_2000_downstream_500_v1" \
    || $14 != 2000 || $15 != 500 \
    || $16 != "tes_upstream_500_downstream_2000_v1" \
    || $17 != 500 || $18 != 2000 || $19 != "cofactor_context") { exit 1 }' \
    "$temporary/batched/plan/context_tasks.tsv"
...
awk -F '\t' 'NR > 1 && $19 != "anchor_annotation" { exit 1 }' \
