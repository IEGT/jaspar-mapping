...
m.contexts.num.binary.confirmed.ta.sum <- count.cofactors.per.tfbs(confirmation=c("tp73"), TA.or.DN="TA", threshold.num.reads=threshold.num.reads)
m.contexts.num.binary.confirmed.ta.sum.total <- rowSums(m.contexts.num.binary.confirmed.ta.sum, na.rm = TRUE)
m.contexts.num.tfbs.confirmed.ta <- count.tfbs(confirmation=c("tp73"), TA.or.DN="TA", threshold.num.reads=threshold.num.reads)
m.contexts.num.tfbs.confirmed.ta.total <- sum(m.contexts.num.tfbs.confirmed.ta, na.rm = TRUE)

m.contexts.num.binary.confirmed.dn.sum <- count.cofactors.per.tfbs(confirmation=c("tp73"), TA.or.DN="DN", threshold.num.reads=threshold.num.reads)
m.contexts.num.binary.confirmed.dn.sum.total <- rowSums(m.contexts.num.binary.confirmed.dn.sum, na.rm = TRUE)
m.contexts.num.tfbs.confirmed.dn <- count.tfbs(confirmation=c("tp73"), TA.or.DN="DN", threshold.num.reads=threshold.num.reads)
...
m.contexts.num.binary.confirmed.taOrDn.methylation.sum <- count.cofactors.per.tfbs(confirmation=c("tp73","pos"), TA.or.DN="any", threshold.num.reads=threshold.num.reads)
m.contexts.num.binary.confirmed.taOrDn.methylation.sum.total <- rowSums(m.contexts.num.binary.confirmed.taOrDn.methylation.sum, na.rm = TRUE)
m.contexts.num.binary.confirmed.ta.methylation.sum <- count.cofactors.per.tfbs(confirmation=c("tp73","pos"), TA.or.DN="TA", threshold.num.reads=threshold.num.reads)
m.contexts.num.binary.confirmed.ta.methylation.sum.total <- rowSums(m.contexts.num.binary.confirmed.ta.methylation.sum, na.rm = TRUE)
m.contexts.num.binary.confirmed.dn.methylation.sum <- count.cofactors.per.tfbs(confirmation=c("tp73","pos"), TA.or.DN="DN", threshold.num.reads=threshold.num.reads)
m.contexts.num.binary.confirmed.dn.methylation.sum.total <- rowSums(m.contexts.num.binary.confirmed.dn.methylation.sum, na.rm = TRUE)
m.contexts.num.tfbs.confirmed.taOrDn.methylation <- count.tfbs(confirmation=c("tp73","pos"), TA.or.DN="any", threshold.num.reads=threshold.num.reads)
m.contexts.num.tfbs.confirmed.taOrDn.methylation.total <- sum(m.contexts.num.tfbs.confirmed.taOrDn.methylation, na.rm = TRUE)
m.contexts.num.tfbs.confirmed.ta.methylation <- count.tfbs(confirmation=c("tp73","pos"), TA.or.DN="TA", threshold.num.reads=threshold.num.reads)
m.contexts.num.tfbs.confirmed.ta.methylation.total <- sum(m.contexts.num.tfbs.confirmed.ta.methylation, na.rm = TRUE)
m.contexts.num.tfbs.confirmed.dn.methylation <- count.tfbs(confirmation=c("tp73","pos"), TA.or.DN="DN", threshold.num.reads=threshold.num.reads)
m.contexts.num.tfbs.confirmed.dn.methylation.total <- sum(m.contexts.num.tfbs.confirmed.dn.methylation, na.rm = TRUE)
...
my.volcano.plot <- function(data,title="Volcano Plot: Enrichment vs Frequency",
    ...
    ggplot(data, aes(x=Enrichment, y=Frequency, label=TF)) +
        geom_point(aes(fill=TAvsDN), color="black", size=1.5, shape=21, alpha=0.75) +
...
pdf(pdf.volcano.filename, width=13, height=9)
...
relevant.cofactors <- TRUE & abs(enrichment.ta.human) > 0.1 & frequency.cofactors.ta.human > 0.05
...
write.xlsx(list(TA=volcano_data.ta[,-4],DN=volcano_data.dn[,-4]), file="volcano_data.xlsx", rowNames=TRUE)


relevant.cofactors <- relevant.cofactors | abs(enrichment.dn.human) > 0.1 & frequency.cofactors.dn.human > 0.05
